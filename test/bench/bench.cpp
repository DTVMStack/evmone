...
void register_benchmarks(std::span<const BenchmarkCase> benchmark_cases)
{
    ...
    if (const auto it = registered_vms.find("external"); it != registered_vms.end())
        external_vm = &it->second;
    ...
    for (const auto& b : benchmark_cases)
    {
        ...
        for (const auto& input : b.inputs)
        {
            ...
            if (external_vm != nullptr)
            {
                const auto name = "external/execute/" + case_name;
                RegisterBenchmark(name, [&vm = *external_vm, &b, &input](State& state) {
